...
function initMatrix3dFollow($els) {
    document.onmousemove = function (event) {
        $els.each(function() {
            var e = event || window.event;
            var x = (e.pageX - cumulativeOffset(this).left - 350 / 2) * -1 / 100;
            var y = (e.pageY - cumulativeOffset(this).top - 350 / 2) * -1 / 100;

            var matrix = [[1, 0, 0, -x * 0.00005],
                          [0, 1, 0, -y * 0.00005],
                          [0, 0, 1,            1],
                          [0, 0, 0,            1]];
            ...
            this.style.transform = 'matrix3d(' + matrix.toString() + ')';
        })
    };
}